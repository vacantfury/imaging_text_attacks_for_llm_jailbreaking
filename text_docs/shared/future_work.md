...
**STATUS (2026-07-16) — the best-of-N defense (Paper D) is LIVE but GATED on the R4 experiment (§4.3); it has NOT been fairly tested.** The 2026-07-16 pilot ran the *wrong architecture* — canonicalize → *target*, no guard, no adaptive attacker — and (unsurprisingly) found that cleaning the input helps the raw target comply. The actual thesis is canonicalize → **guard**: collapse the N surface variants to one canonical form and let a guard block it, giving **N-independent protection on the canonicalizable classes** — a slope/exponent kill, qualitatively different from DATDP's constant ~500× multiplier (Armstrong et al., 2025; a weak, non-adaptive, expensive, single-benchmark incumbent). Whether it is a good paper turns on one unrun experiment (R4): does forcing an **adaptive** attacker off the canonicalizable head onto the irreducible tail bend the aggregate ASR-vs-N slope, or only shift the constant? Full record: `text_docs/bestofn_defense/proposal.md §9`.
...
High-variance / bimodal — strong upside (first credible defense to a prominent, currently-undefended attack, with a super-constant work-factor increase) vs. a weak floor (constant-factor only → adaptive evasion → a negative result that is hard to place), plus **scoop risk** (best-of-N is a hot target). But the variance is **front-loaded and cheaply gated**: one experiment — does the defense change the power-law's *constant* or its *slope*? — resolves most of it before any real commitment. A **gated future option**, eval-only on the open-VLM cluster; run the gate first, commit only if the cost-increase scales super-constant. Full defense is *not* a goal (it would require solving black-box adversarial robustness). *(2026-07-16: the gate has NOT been fairly run — the pilot used canonicalize→target, not canonicalize→guard, and no adaptive attacker; the proper R4 gate is still pending. See STATUS above.)*

### 4.4 Broader generalization — the meaning-preserving-perturbation class (a separate direction)

Best-of-N is one instance of a broader class — **meaning-preserving perturbation attacks** (surface noise, character-level obfuscation, cipher / homoglyph / ASCII-art, and — semantically — encodings and cross-modal splits). A single recover/canonicalize-then-guard defense covers the class *in principle*, but the **defense mechanism is not open ground**: the surface end is the decode-then-guard crowd (DecipherGuard, RLM-JB, Broken-Token, CPT-filtering, TRYLOCK) and the semantic end is our own coverage-complete line (§2 / Paper C). Generalizing the *defense* therefore **subtracts** novelty rather than adding it — the best-of-N-specific niche was the only unowned sliver.
...
> **✅ PROMOTED to Paper E, then SPUN OUT to its own repo (2026-07-19).** This agent-side line became Paper E "Smuggled Actions", then moved to its **own repo `llm_agent_security`** — the agent runtime (tool-loop, external harness, action-scoring) shares ~none of this repo's VLM eval pipeline (only the encoders, copied there). §5.1 (encoded injection, attack-first) is that paper's core; §5.2 (defense) and §7.2 (multi-agent) are its future work. This section is retained as the origin record; live work is in the sibling repo.
...
## 9. Judge reliability on encoded/rendered harm — an evaluation-methodology paper (candidate spin-out)  [idea]

> **⏸ PARKED (2026-07-19)** — investigated as Paper E (`judge_reliability` namespace, codename "Blind Judges"). Two independent scoop-checks returned **Medium/High overlap** (judge-calibration = StrongREJECT/SORRY-Bench; capability-gap = the scalable-oversight scaling laws — Engels/Dorner/Kenton), so it was **not pursued as a standalone Paper E**; the E alias moved to `agent_injection` (§5). Its measured judge-validation stays in Papers C/D. Retained as a dormant candidate. Full disposition: `text_docs/judge_reliability/proposal.md §11`.

**The gap (DOUBLY-confirmed by the 2026-07-12 open-judge landscape searches):** no published work directly, quantitatively measures how much automated safety **JUDGES under-count harm when the harmful content is ENCODED** (cipher / base64 / set-theory / formal-logic / code / classical-Chinese) **or IMAGE-RENDERED**. The literature has only *adjacent* pieces — judges fragile to surface/style (Know Thy Judge, arXiv 2503.04474), anchoring on malicious *tone* → false positives (Confusion is the Final Barrier, arXiv 2508.16347, EMNLP'25 Findings), degrading under distribution shift (A Coin Flip for Safety, arXiv 2603.06594), and *qualitative* notes that classifiers miss obfuscated responses (h4rm3l 2408.04811, CodeChameleon 2402.16717, cipher-vs-guardrails 2405.20413). Nobody has run the systematic **per-judge × per-encoding × human-labeled** measurement.

**The paper:** N judges (fine-tuned classifiers: `harmbench`-cls / `llama_guard` / `wildguard` / MD-Judge · reasoning guards: GuardReasoner / ThinkGuard · LLM-judges: gpt-5-nano + a capable open model with a rubric · multilingual: Qwen3Guard / PolyGuard / ShieldLM) × M encoder types (our set-theory / formal-logic / code / classical-Chinese + cipher/base64 + image-rendered) × human-labeled ground truth → measure the **under-count (miss rate)** and over-count per (judge, encoding), and whether a **decode-then-judge** step recovers it. Contribution = the first calibration of judge reliability on encoded/rendered harm + a practical which-judge / decode-first recommendation.

**Relation to Paper C:** this *is* Paper C's Round-J judge validation, run richly. **Decision (DEFER — owner flagged standalone potential 2026-07-12):** design Round J as a proper measurement so it can EITHER (a) fold into Paper C as a first-of-its-kind methodology contribution, OR (b) spin out as a standalone eval-methodology paper if the results are strong. Decide once the numbers are in — low-regret, since the work is needed for Paper C either way.

**Fit:** on-thesis for AI-safety *evaluation* (a distinct axis from the defense contribution), a genuine open gap, an active area (many 2026 judge-reliability papers), and it reuses infra already built (`conf/experiment/harmbench_judge.yaml`, the guard/judge registry in `llm_utils`, the human-κ pipeline). See `literature_review.md` §7.5 and the `project_paper_c_judge_model_open_problem` memory.
