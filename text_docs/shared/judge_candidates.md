...
- **Capability arm — PROMOTED to active (owner-approved 2026-07-15; AICR unlocks it):** `Qwen/Qwen3-235B-A22B-Instruct-2507` (capability + strongest Chinese → the classical-Chinese arm) · `deepseek-ai/DeepSeek-V3.2-Exp` (capability + permissive + judge precedent 2603.17368; native FP8). No longer "Phase 2 deferred" — AICR serves them via 8-GPU tensor-parallel (no NURC fp8-on-1-GPU workaround). Downloading to AICR only (jobs 157576/157577); serving configs are AICR-multi-GPU, and DeepSeek-V3.2's sparse-attention needs a vLLM serve-smoke first. Even higher (Kimi-K2 1T / DeepSeek-V4-Pro 1.6T) stays API-only/deferred.
