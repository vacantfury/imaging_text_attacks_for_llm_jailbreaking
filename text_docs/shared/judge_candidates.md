...
- **Capability arm — PROMOTED to active (owner-approved 2026-07-15; 3 models; AICR unlocks it):** `Qwen/Qwen3-235B-A22B-Instruct-2507` (capability + strongest Chinese → the classical-Chinese arm) · `deepseek-ai/DeepSeek-V3.2-Exp` (capability + permissive + judge precedent 2603.17368; native FP8) · `moonshotai/Kimi-K2-Instruct` (the PERMISSIVE-CEILING arm — lowest documented general-harm refusal; 1T/32B block-fp8, one b200 node). No longer "Phase 2 deferred" — AICR serves them via 8-GPU tensor-parallel (no NURC fp8-on-1-GPU workaround). **All three DOWNLOADED + verified** (2026-07-15). Serving is AICR multi-GPU (`num_gpus: 8`); serve-smoke each first (DeepSeek-V3.2 sparse-attention, Kimi-K2 size-at-ceiling). **All open-license (not gated).** DROPPED after consideration: `Mistral-Large-3` (riskiest serve — 675B on-the-fly-fp8 OOM — + slow download; permissive axis already covered by Kimi-K2 + DeepSeek-V3.2) and `Llama-4-Maverick` (its only distinct axis was native-multimodal image-judging, unused because Round-J judges score response TEXT only; Meta already covered by Llama-3.3-70B). Even higher (DeepSeek-V4-Pro 1.6T / GLM-5.x) stays API-only/deferred.
