...
- **Open (downloading — see `cluster_models.md`):** already-served `harmbench-cls` / `wildguard` / `llama-guard-4` / `guardreasoner-vl` / `llama-3.1-8b` / `llama-3.3-70b`, plus new pulls `qwen3guard-gen-8b` · `shieldlm-7b` · `md-judge-v0.1` · `thinkguard` · `hermes-4-70b` · `command-a` · `glm-4.5-air`.
- **Capability arm — PROMOTED to active & EXPANDED to 4 (owner-approved 2026-07-15; AICR unlocks it):** `Qwen/Qwen3-235B-A22B-Instruct-2507` (capability + strongest Chinese → the classical-Chinese arm) · `deepseek-ai/DeepSeek-V3.2-Exp` (capability + permissive + judge precedent 2603.17368; native FP8) · `moonshotai/Kimi-K2-Instruct` (the PERMISSIVE-CEILING arm — lowest documented general-harm refusal; 1T/32B block-fp8, one b200 node) · `mistralai/Mistral-Large-3-675B-Instruct-2512` (Western MINIMAL-ALIGNMENT baseline; Apache-2.0, 675B/41B fp8). No longer "Phase 2 deferred" — AICR serves them via 8-GPU tensor-parallel (no NURC fp8-on-1-GPU workaround). Qwen3-235B/DeepSeek-V3.2 downloading (jobs 157576/157577); Kimi-K2/Mistral-Large-3 downloads pending. Serving is AICR multi-GPU (`num_gpus: 8`); serve-smoke each first (DeepSeek-V3.2 sparse-attention, Kimi-K2 size-at-ceiling, Mistral-Large-3 on-the-fly-fp8 OOM risk). **All four open-license (not gated).** `Llama-4-Maverick` was considered and DROPPED — its only distinct axis was native-multimodal image-judging, unused because Round-J judges score response TEXT only; Meta already covered by Llama-3.3-70B. Even higher (DeepSeek-V4-Pro 1.6T / GLM-5.x) stays API-only/deferred.
