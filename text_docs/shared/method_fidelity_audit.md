...
| **CIDER** | Mechanism transcribed from the authors' code; embedding space corrected to the MLLM's own (2026-08-05); one declared deviation (DnCNN single-pass instead of guided diffusion ×7 checkpoints) that strictly *weakens* the detector and is flagged as a lower bound. `src/defense/vendor/guided_diffusion/` is untracked in the tree — that deviation is being closed. |
