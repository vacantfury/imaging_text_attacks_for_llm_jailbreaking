...
| **AS-2** | **B** | Presence Tax | *The Presence Tax: Vision–Language Models Condition Refusal on Image Attachment Rather Than Harm* — safety-aligned VLMs key their refusal threshold on whether an image is attached, a feature carrying zero harm-information; the cost lands on sensitivity-adjacent benign traffic and is decoupled from any safety benefit | `presence_tax` | *(working docs local-only)* | under review |
...
| **AS-7** | — *(born ID-first)* | Read Access | *What the Defense Can Read: Channel Scope and Evaluation Protocol in Black-Box Multimodal Guardrails* — what an input defense can read decides what it catches (text-only guards miss a payload that merely moved to the image channel), and what the evaluation lets it read decides how safe it scores (oracle protocol inflates measured benefit) | `read_access` | *(working docs local-only)* | in progress |
