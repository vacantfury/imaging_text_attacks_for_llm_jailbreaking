...
| **D** | BoN Canonicalization | Canonicalization defense against **Best-of-N** jailbreaking — collapse the attacker's *effective N* by canonicalizing inputs (not lowering per-try p); carve a canonicalizable head vs an irreducible tail | `bestofn_defense` | `text_docs/bestofn_defense/proposal.md` | in progress *(code built, pilot pending)* |
