...
| **AS-7** | — *(born ID-first)* | Whose Refusal Is It? | *Whose Refusal Is It? The Attacker and the Harness Set What a Black-Box Guardrail Reads* (retitled 2026-08-21; this row was TWO retitles stale, still carrying the 2026-08-08 "Read Access: Channel Coverage and Oracle Inflation…" and never picking up the 2026-08-10 "…The Attacker Decides How Much a Black-Box Guardrail Contributes". **`paper.tex` is the authority and this row follows it — sync it in the same session as any retitle.** The 2026-08-21 title names BOTH parties that move the guard's read, where the 08-10 one named only the attacker, and drops "Contributes", which reviewers twice read as an efficacy claim the refusal decomposition does not license) — what an input defense can read decides what it catches (text-only guards miss a payload that merely moved to the image channel), and what the evaluation lets it read decides how safe it scores (oracle protocol inflates measured benefit) | `defense_read_access` | *(working docs local-only)* | in progress |
