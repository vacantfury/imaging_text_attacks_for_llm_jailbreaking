...
| **AS-7** | — *(born ID-first)* | Whose Refusal Is It? | *Whose Refusal Is It? Reported Guardrail Safety Measures the Pipeline, Not the Guardrail* (retitled 2026-08-21, SECOND pass same day, superseding "…The Attacker and the Harness Set What a Black-Box Guardrail Reads"; the row had also been two retitles stale before that. **`paper.tex` is the authority and this row follows it — sync it in the same session as any retitle.** Reasoning for the second pass: after FOUR consecutive cspaper 4/rejects at confidence 4, where targeted fixes killed their cons without moving the score, the diagnosis is framing rather than argument. The morning title stated the MECHANISM (what the guard reads); the paper's finding is the CONSEQUENCE (the reported number is not a measurement of the guard). The new title also concedes the obvious objection in advance — the pipeline number is legitimate *as a pipeline number* — so the claim is misattribution, not uselessness) — what an input defense can read decides what it catches (text-only guards miss a payload that merely moved to the image channel), and what the evaluation lets it read decides how safe it scores (oracle protocol inflates measured benefit) | `defense_read_access` | *(working docs local-only)* | in progress |
