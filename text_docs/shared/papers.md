...
| **AS-2** | **B** | Presence Tax | *The Uncontrolled Variable: Safety-Aligned Vision–Language Models Key Refusal on Request Form, Not Content* — safety-aligned VLMs key their refusal threshold on properties of a request's FORM that carry no information about what is asked; image attachment is the instance with a deployment surface, and an asserted attachment moves one checkpoint through the text channel alone. The cost lands on sensitivity-adjacent benign traffic, is decoupled from any safety benefit, and its sign is not fixed. ⚠️ **Codename ≠ title since 2026-08-08:** the codename stays `Presence Tax` (registered here and in the science portfolio); the TITLE dropped the tax metaphor because the exchange rate was withdrawn as unreproducible and the sign inverts on `pixtral-12b`. Do not 'fix' either to match the other. | `image_presence_threshold` | *(working docs local-only)* | under review |
