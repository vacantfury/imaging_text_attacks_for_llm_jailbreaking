...
| **AS-2** | **B** | Presence Tax | *The Presence Tax: Vision–Language Models Condition Refusal on Image Attachment Rather Than Harm* — safety-aligned VLMs key their refusal threshold on whether an image is attached, a feature carrying zero harm-information; the cost lands on sensitivity-adjacent benign traffic and is decoupled from any safety benefit | `image_presence_threshold` | *(working docs local-only)* | under review |
