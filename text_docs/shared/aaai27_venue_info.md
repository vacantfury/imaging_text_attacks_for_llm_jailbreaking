...
- **Track-specific deadline — page now LIVE, dates still TBA (checked 2026-07-20):** the dedicated AAAI-27 AI Alignment CFP page (https://aaai.org/conference/aaai/aaai-27/aia-call/) is up but the CFP + important dates are **not yet released** — *"The Call for Papers (CFP), submission instructions, and important dates for the AI Alignment Special Track will be announced within the next few days"*; the timetable lists all deadlines as **TBA** with the note *"Deadlines are track-specific and may differ from those for other tracks,"* and the **AIA submission site is separate and not yet open**. Prior-year pattern is mixed (AAAI-26 shared the main-track dates; AAAI-25 ran a few days later than main) → **do NOT assume 7/21/7/28; WATCH the page daily until the CFP drops** (TODO item 1 leading watch), then capture abstract/full dates + page limit + rubric. Because the AIA site may not be open by our 7/20 abstract plan, the fallback is to register the free non-binding **main-track** abstract on 7/20 and, if we later choose AIA and its abstract deadline still allows, register fresh on the AIA site (letting the main-track placeholder lapse) — viable only because the main-track abstract is non-binding, but note it commits the TRACK, so a paper firmly intended for AIA should wait for the AIA site if its deadline permits.
