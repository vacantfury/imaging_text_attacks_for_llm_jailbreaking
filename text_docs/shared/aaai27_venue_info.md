...
- **arXiv version always links the REAL public repo** (`github.com/…/llm_guardrail_security`), never the blind review mirror (`blind-submission1111/…`, which expires). What the anonymized submission carries is **venue-dependent (corrected 2026-08-02; verified 2026-07-28, memory `workflow_anon_repo_link_at_paper_done`)**: ARR/ACL allows an anon-repo link in the submission; **AAAI BANS anon-repo links** — code goes in as an uploaded supplementary ZIP, and the submission PDF carries no repo pointer at all. The repo is already public under the author's name (project policy), so linking it on arXiv adds no de-anonymization beyond the named preprint itself.
...
| Camera-ready | Dec 14, 2026 |
  ...
  - **Still pending on the page as of 2026-08-02 (re-checked; unchanged since 2026-07-29):** the CFP text proper (page now reads *"The Call for Papers (CFP), submission instructions, and important dates for the AI Alignment Special Track will be announced within the next few days"*) — so the AIA-27 page limit, rubric confirmation, appendix/supplementary policy, and submission instructions are NOT yet published — and the **separate AIA submission site is still NOT open**. Keep the watch until both land; Paper B's appendix split (`xr` `\externaldocument`) and any page-budget work stay gated on it.
  ...
  - **Decision-clock consequences** (recorded in TODO item 1): the C/D withdraw-from-main-for-AIA decision lands ~2 days before the **8/14** AIA abstract wall (~8/12), gated on the chairs' answer + CFP text + site opening. **Paper B: SETTLED 2026-08-01/02 — committed to EMNLP AND holding an AIA option; the two compose** (owner's find + ruling 2026-08-01, superseding this doc's earlier "committing forecloses AIA" claim): an AIA **abstract registration is a free non-binding placeholder** (title/abstract/authors only, not a paper "under review"), so B registers an abstract 8/12; the binding conflict triggers only at the **full paper** (8/21), and EMNLP notifies 8/20 — accepted → the abstract lapses; rejected → file B's AIA full paper 8/21. Per the recorded ④ exception, B is review-complete after the meta-review → no ARR withdrawal needed for an AIA submission (re-verify that reading at decision time).
...
- ✅ **SETTLED 2026-08-01 (owner ruling): treat abstract registration as NOT a submission** — it is common conference practice and AAAI states no explicit ban. The AAAI-27 call page is silent on the point (fetched 2026-08-01); the ruling stands on practice + absence of a ban, and the question is closed — do not re-open it or write to the chairs about it.
- **Scheduling rule (owner 2026-08-01, GLOBAL — supersedes the one-day version above): every deadline-bound action lands ≥2 DAYS before the wall**, never on deadline day and never with a one-day buffer. So an 8/14 abstract wall is executed **8/12**. Canonical home: global `~/.claude/CLAUDE.md` §Owner-interaction defaults.
- **A retitled paper is NOT a distinct paper for dual-submission purposes** — AAAI's bar is on work that does *"not constitute distinct scientific contributions"*, so timing is what separates two routes, never the title. Corollary the other way: an abstract registered for a paper under active refinement should be written FRESH to describe the refined contribution, rather than pasted from the earlier venue's title/abstract.
