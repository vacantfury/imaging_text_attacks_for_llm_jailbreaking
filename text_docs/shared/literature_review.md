...
> **Part 0 (added 2026-05-31) positions Paper C** — *adversarial modality placement; the modality-complete guard; cross-modal splitting → joint verification* — against the most directly related recent work, with explicit threat/baseline calls. The broader survey (foundational jailbreaking, typographic attacks, encoding attacks, defenses, evaluation) follows from §1 (this is the earlier Paper-B-era review, still relevant background).

## 0. Paper C focus — modality coverage & the VLM defense landscape

### 0.1 Conceptual basis: the modality gap
Liang et al. (NeurIPS 2022), *Mind the Gap*, show that in CLIP-style joint embedding spaces, image and text embeddings occupy **separate narrow cones** — they never interleave; the gap exists at initialization (the "cone effect") and is preserved by contrastive learning's low temperature. **For C:** the canonical citation that the two modalities are *structurally separable representations*, motivating why a black-box guard inspecting a projection of one channel (text tokens, or an image→caption summary) has a structural blind spot on the other. **Honest caveat:** it concerns contrastive *dual-encoders* (CLIP), whereas generative VLMs fuse a vision encoder into an LLM — cite as motivating intuition for separability, not a literal property of the target's internal space. No safety/attack content → pure support, no threat.

### 0.2 The question C must answer head-on: does text-side safety cover the image channel?
Chakraborty, Shayegani et al. (EMNLP Findings 2024), *Can Textual Unlearning Solve Cross-Modality Safety Alignment?*, apply machine unlearning to **only the LLM component** of a frozen-vision VLM, cutting vision-text-attack ASR from 54–91% to ~2–8% (LLaVA-1.5/1.6; FigStep, Jailbreak-in-Pieces, JailbreakV), beating multimodal SFT at ~6× lower cost. Thesis: all inputs project into the **language embedding space**, so a text-side intervention governs generation regardless of input modality.

Its title literally answers C's question, so **C must address it explicitly** (silence reads as oversight):
- **Different layer.** This is *training-time white-box unlearning*; C targets *black-box inference-time guards* (SAGE, ECSO, ETA). It does not bear on whether a black-box guard covering one modality is bypassable.
- **Ally, not competitor.** Its "fusion into language space" premise is exactly what makes C's modality-complete guard plausible (recover both channels into one text-space check).
- **Untested regime = C's gap.** Its attacks are *non-adaptive* (fixed corpus); its own Limitations concede harder/adaptive attacks are untested. C's adaptive *modality placement* and *cross-modal splitting* (each channel individually benign) are that gap — and a text-side prior has nothing harmful to suppress in a split where no single channel is harmful.
- **Verdict:** cite as (i) support for the fusion premise behind the guard and (ii) the foil whose untested gap C fills; **not a baseline** (wrong layer — requires retraining, forbidden in C's black-box setting). Positioning line: *"Prior work shows text-side training transfers to in-distribution image attacks (consistent with our fusion premise); we show black-box inference-time defenses covering only one modality remain bypassable under adaptive placement — a regime that work leaves untested."*

### 0.3 Closest related work — inference-time VLM defenses (C's real baselines)
C's `modality_complete` / `joint_verify` are inference-time check-then-gate defenses; **C should benchmark against these, not only SAGE/ECSO.**

- **ETA (Ding et al., ICLR 2025) — the most important baseline (and a partial threat).** Training-free, two-phase: *Evaluate* (a CLIP pre-generation check of the **visual input** against unsafe categories + a reward-model check of the **output**) then *Align* (interference prefix + sentence-level best-of-N). Cuts cross-modality adversarial USR by **87.5%** (vs ECSO's ~5%), utility preserved, +0.1s/gen. Diagnostic: *"bypassing of safety in VLMs is primarily due to the continuous nature of visual token embeddings"* — a framing C should note as **complementary but distinct** from "incidental modality coverage." **Why it threatens C:** unlike SAGE (text-only) and ECSO (`has_image`-gated), ETA *does inspect the image* — a reviewer will ask "doesn't ETA already cover the image?" **Why C survives (but must prove it):** ETA's image check is a *coarse CLIP-category similarity*, not a unified joint check — (a) cross-modal **splitting** can stay below its threshold (each channel benign to CLIP); (b) it gates only when **both** evaluators fire; (c) CLIP is weak at reading *rendered/typographic* harmful text. **Defeating ETA is a far stronger result than defeating SAGE/ECSO → add ETA as a target defense.**
- **BlueSuffix (ICLR 2025) — strongest bimodal black-box defense.** DiffPure image purifier + LLM text purifier/rewriter + RL "blue-team" safety suffix; drives many attacks to ~0% ASR, transfers black-box to Gemini. Touches both modalities but is **not modality-complete in C's sense** (purify/rewrite + nudge self-refusal; no recover-and-unified-check). **Gap:** the image purifier targets adversarial *perturbations* (denoising) and does nothing to *rendered text* in the image; the text purifier never sees it. C's placement/splitting in the clean-image-text channel is unaddressed → compare against it; show empirically.
- **MLLM-Protector (EMNLP 2024) — the output-side class.** Harm detector (3B) + detoxifier operating **strictly on the model's text output** (never the image/input). Orthogonal to input guards: C's placement doesn't "hide in an uncovered input channel" here; the relevant evasion is C's `ecso_evade`-style **output framing**. Cite as the canonical output-side defense; include to show neither input-coverage nor naive output-checking alone suffices.

### 0.4 Defense taxonomy & C's positioning

| Defense | Class | Modalities covered | C's evasion |
|---|---|---|---|
| SAGE | input · text-only | text channel | place content in image |
| ECSO | input · caption-gated | image→caption (gated on `has_image`) | re-check evasion / split |
| **ETA** | input · eval-then-align | image (coarse CLIP) + output | split below CLIP threshold; rendered text |
| BlueSuffix | input · bimodal purify | image (perturbation) + text (rewrite) | clean rendered-text channel; split |
| MLLM-Protector | **output**-side | model output only | output framing |
| textual unlearning | **training**-time | fused representation | (different layer; non-adaptive only) |
| **`modality_complete` (C)** | input · recover-both | both → unified check | (the fix; only splitting defeats it) |
| **`joint_verify` (C)** | input · joint | joint (text, image) | (the fix for splitting) |

**C's contribution survives:** no prior defense does *"recover BOTH channels → one unified eyes-closed safety check"* (`modality_complete`), and none does *joint multimodal verification* against cross-modal splitting. ETA's CLIP guard is the closest but is coarse and per-modality. Novelty holds **provided** C (a) compares against ETA, not just SAGE/ECSO; (b) shows splitting defeats even ETA; (c) frames the modality-coverage principle distinctly from ETA's embedding-continuity and textual-unlearning's fusion framings.
...
### 3.17 DeepSeek-OCR: Contexts Optical Compression (Wei et al., 2025)

DeepSeek-OCR is not a jailbreak or safety paper, but it is highly relevant background for why image-rendered text should be studied as a first-class input representation. The paper proposes **contexts optical compression**: rendering textual content into images and using a vision encoder, DeepEncoder, to compress the visual representation into a small number of vision tokens before decoding it with a compact MoE language decoder. The central claim is that the visual modality can serve as an efficient compression medium for textual information, not merely as a source of natural-image features.

The paper reports strong compression-accuracy tradeoffs: when text tokens are within roughly 10× the number of vision tokens, DeepSeek-OCR achieves around **97% OCR precision**; at 10-12× compression, precision remains around **90%**; even near 20× compression, it retains about **60%** accuracy. On OmniDocBench, DeepSeek-OCR uses far fewer vision tokens than many end-to-end OCR/document parsing baselines while remaining competitive, e.g., surpassing GOT-OCR2.0 with 100 vision tokens and outperforming MinerU2.0 with fewer than 800 vision tokens.

**Relevance to our study:** DeepSeek-OCR changes the motivation for our work. Text rendered as images is not only an attack trick like FigStep; it is an emerging **systems design pattern** for long-context compression, memory, document parsing, and future agent infrastructure. If models increasingly route textual content through optical/OCR pathways, then safety cannot be assumed to transfer from tokenized text to image-rendered text. Our project studies exactly this missing safety question: when harmful or encoded text is optically represented, do VLMs preserve text-channel safety behavior, become more permissive, or become more conservative?

**Important distinction:** DeepSeek-OCR evaluates reconstruction and document understanding, not safety. It shows that optical text representations can preserve semantic content efficiently; it does not test whether safety filters, refusal behavior, or jailbreak robustness survive this representation change. This creates a clear gap for our work: **safety under optical text compression and image-rendered textual representations**.

### 3.18 ComicJailbreak (Tan et al., 2026)

**ComicJailbreak** (Tan, Hu, & Lee, 2026) introduces a **structured visual narrative** jailbreak benchmark with 1,167 attack instances across 10 harm categories and 5 task setups. The attack converts harmful prompts into multi-panel comic templates, embedding the malicious narrative into a visually structured story rather than a single typographic image. Comic-template attacks demonstrate ASR comparable to rule-based jailbreaks across SOTA MLLMs. **Relevance:** ComicJailbreak is conceptually adjacent to our work — it exploits the same vision-pathway weakness that typographic attacks rely on, but adds *narrative structure* (comic panels) as an additional layer of visual context that breaks the assumption of single-image typographic attacks like FigStep. A particularly important finding from the paper for our defense framing: existing safety defenses "induce a high refusal rate on benign prompts" — directly supporting our argument that defense calibration (the over-refusal trade-off) is the central tension for any new defense in this space. The comic-narrative dimension is orthogonal to our encoding × modality design; in principle, both directions could be combined (encoded prompts rendered as comic panels) — a future-work direction we do not pursue in this paper.

### 3.19 Reference Attack (Wang et al., ACL 2026)

Reference Attack (Wang, Fu, & Gao, BUPT — **ACL 2026, Long Papers**, `2026.acl-long.812`) is a **black-box, reference-guided cross-modal jailbreak** that enhances prompt-to-image injection by exploiting the MLLM's own semantic-reconstruction (reasoning) capability rather than shallow image-level deception. Three steps: (1) **Prompt Mixing** — extract the malicious words from the harmful prompt, replace them with benign symbols (`*`), and shuffle the disguised prompt into a set of benign prompts; (2) **Reference Construction** — build a chain of `n` references `r₁ → r₂ → … → rₙ` where each points to the next and the last points to the disguised malicious prompt, plus a *mapping sentence* `M(T_b, S_m)` that (benignly) maps the symbols back to the malicious words, so the model reconstructs the harmful query by **layered reference resolution** instead of reading it directly; (3) **Non-Text Modality Conversion** — everything except the first (benign) text prompt is rendered into a **spreadsheet or image**, placing the malicious content and the reference chain in the modality content moderation scrutinizes least. A theoretical attention-decoding analysis predicts ASR rises with reference-chain length, validated empirically (ChatGPT-4o: **18.6% at length-0 → 97.6% at length-3**). Evaluated on SafeBench (500 queries) and MM-SafetyBench (5,040 pairs) under a 3-critic majority-vote ASR protocol (human-validated, deviations <1.5%): **SafeBench ASR — ChatGPT-4o 97.6%, Gemini-2.0-Flash 98.4%, Claude-3.5-Sonnet 97.4%, LLaMA-3.2-11B 94.4%** (all >93%), beating FigStep / FigStep-Pro / Visual Prompt / Combination Attack by up to **+70.8%**. Appendix H sketches preliminary defenses: **cross-modal consistency checks and reference-chain bounding**.

**Direct relevance to Paper C — the peer-reviewed multimodal attack the coverage-complete guard should defend against.** Reference Attack is exactly the image-channel threat `modality_complete` (recover-and-decode across modalities → guard) is built to close: it hides the payload in the image/spreadsheet modality and forces the model to *recover* harmful semantics from cross-modal references — the same recover-then-inspect gap our guard targets, weaponized. Two consequences: (i) it is a strong, **published, third-party target attack** (ASR >93% on frontier COTS models) — a far more defensible image-channel motivation than a bare `ir_plain` render or a preprint (CAMO), because an ACL-2026 result *establishes* that cross-modal reference resolution bypasses deployed moderation; (ii) its Appendix-H defense sketch (cross-modal consistency + reference-chain bounding) is the **closest prior defense idea to ours** and must be differentiated — our contribution *decodes* (restates the obfuscated payload to plaintext before a general guard) and is a preprocessing **amplifier prependable to any guard**, not an attack-specific consistency check. Caveats for adoption: the reference-chain construction is more elaborate than a single encoded-image render (faithful reimplementation of the 3-step pipeline needed; check for released code), and it maps most naturally onto Paper C's **cross-modal-splitting / stretch** direction rather than the base image channel.
...
### 6.2 Safety Classifiers (general-purpose guard baselines)

General-purpose safety classifiers ("guard models") are the strongest *deployed* input/output defenses, and they are the category a reviewer will point to first — so we include the strongest representative of each sub-family as comparison baselines rather than dropping them for implementation cost. Covering them is what makes the "no single deployed defense covers the union" claim airtight: if even a production guard has a hole on the encoding or modality axis, the coverage gap is not an artifact of testing only weak specialists.

**Text guards (architecturally blind to the image channel).** Llama Guard (Inan et al., 2023) operates on **tokenized text**, making it architecturally unable to evaluate harmful content in images without an OCR step — the limitation central to our defense experiments (we test whether OCR/recover → Llama Guard forms an effective pipeline). The same holds for its peers, which we add to show the coverage gap is not Llama-Guard-specific: **ShieldGemma** (Zeng et al., 2024; Gemma-2-based, reports +10.8% AU-PRC over Llama Guard), **WildGuard** (Han et al., NeurIPS 2024; a one-stop tool classifying prompt harm, response harm, *and refusal* — so it doubles as our over-refusal baseline), **ShieldLM** (Zhang et al., EMNLP Findings 2024; bilingual zh/en, relevant to the low-resource-language axis), **Aegis / Aegis 2.0** (Ghosh et al., 2024/2025; NVIDIA's ensemble-of-experts guard), and **MD-Judge** (Li et al., SALAD-Bench, ACL Findings 2024). Every one is a text classifier: on the *image* channel it is structurally blind (it cannot see text rendered into pixels at all), and on the *text* channel the open question is whether a classifier trained on natural-language harm fires on a semantic encoding (set theory, formal logic, Classical Chinese) whose surface form is far from any harmful training example. We expect them defeated on both axes — blind to the image, low-recall on encoded text — which is precisely the gap our recover-then-guard defense is designed to close.

**The deployed multimodal guard — the decisive baseline.** Llama Guard 3 Vision (Chi et al., 2024) is Meta's production multimodal guard, fine-tuned on Llama-3.2-Vision to safety-check *image+text* conversations directly. It is the single most important baseline in the paper: unlike the text guards it *can* see the image, so it is the strongest possible defender of the modality axis. The question it answers is the crux of the motivation — does a real, deployed multimodal guard catch harmful requests that are semantically encoded and then rendered into an image? If Llama Guard 3 Vision is defeated by text-in-image encoded attacks, then "no single deployed guard covers the union" is established against the strongest available opponent, not a strawman.

**Guards are themselves jailbreakable.** Gamma-Guard (Lv et al., EMNLP 2025) confirms on three public benchmarks (BeaverTails, XSTest, AdvBench) that guard models — LLMs fine-tuned for safety — remain vulnerable to jailbreak attacks, tracing the failure to representation shifts that begin in the embedding layer and cascade through the network (its fix: lightweight residual adapters that lift adversarial accuracy from <5% to 95%). This is direct evidence for our motivating premise: a single guard, even a purpose-built one, is not a sufficient defense — which is why coverage (recovering and decoding across channels *before* guarding), rather than a stronger single classifier, is the contribution.

Together these guards serve two roles in our study: (i) as **standalone defense baselines** that the best-of-suite attack should defeat, establishing the coverage gap against strong, deployed opponents; and (ii) as candidate **GUARD components** inside our recover→decode→guard pipeline (the guard model being the final safety check applied to the recovered, decoded union).

**Current-generation guards (2025–2026) — the baseline set has moved.** The 2023–2024 guards above are now a generation behind; a current submission should also baseline against the newer, often *multimodal*, releases. **GuardReasoner-VL** (Liu et al., 2025) is an RL-trained multimodal reasoning guard that judges text, image, and interleaved inputs — the strongest current VLM-guard baseline. **Qwen3Guard** (Zhao et al., 2025) is a 119-language moderation family whose lightweight *output-side* Stream variant matters for our claim (see §6.11). **SingGuard** (2026) is a policy-adaptive multimodal guardrail whose own taxonomy names "typographic, semantic-isomorphic, narrative, and cross-modal joint-risk" as first-class categories — evidence the field already recognizes our axis. (Newer guards released only as deployed model cards — Llama Guard 4, ShieldGemma 2, Granite Guardian 3.x, Nemotron 3.5 Content Safety — can be added as engineering baselines if reviewers ask, but are not cited here.)
...
**Probabilistic Certificate for SmoothLLM / (k,ε)-unstable** (Kumarappan & Mehrotra, NeurIPS 2025 Lock-LLM Workshop; arXiv:2511.18721) revisits SmoothLLM's *certificate* rather than its heuristic. SmoothLLM's formal guarantee rests on a strict **k-unstable** assumption — an adversarial suffix is assumed to fail whenever ≥ k of its characters are perturbed (jailbreak probability drops to exactly zero beyond k). The authors show empirically (Llama2-7B, Vicuna-7B; GCG and PAIR suffixes under RandomSwap/RandomPatch perturbation) that Attack Success Rate instead decays *exponentially to a nonzero floor*, never reaching zero, so the deterministic certificate is overly optimistic in practice. They relax the assumption to **(k,ε)-unstable** — a perturbed prompt with ≥ k character changes still jailbreaks with probability at most ε — and derive new, data-informed lower bounds on SmoothLLM's Defense Success Probability (a binomial in a single-prompt success probability α whose lower bound combines a hypergeometric character-overlap model with an empirical exponential ASR fit); setting ε = 0 recovers the original certificate. **Relevance to our setting:** this is a refinement *within SmoothLLM's own character-fragile regime* — it sharpens the certificate for GCG/PAIR-style suffixes but, by the authors' own scoping, does not address semantic-encoding attacks. It reinforces our positioning: smoothing certificates are defined on the character-instability axis, whereas our valid-language encodings (set theory, formal logic) carry their meaning at the proposition level and survive character perturbation intact — there is no decay-to-floor curve to certify because perturbation never neutralizes the attack in the first place. The (k,ε) framework therefore tightens a guarantee that is *orthogonal to*, not protective against, our encoding attacks.
...
**RLM-JB — Recursive Language Models for Jailbreak Detection** (Shavit, 2026; arXiv:2602.16520, Silverfort) reframes jailbreak *detection* as a bounded, auditable **procedure** rather than one-shot classification. A root model (an LLM orchestrating a sandboxed analysis program) (1) **normalizes and de-obfuscates** the input — lightweight canonicalization of whitespace/separators plus base64-like decoding — (2) **chunks** the text to guarantee coverage and counter long-context hiding / "lost-in-the-middle," (3) screens each segment in parallel with worker models emitting malicious/benign + confidence + extracted attack vectors, and (4) **aggregates conservatively** across chunks to recover split-payload attacks (any-segment-malicious → malicious). Threat model: tool-augmented agents in stateful environments (email, workflows) ingesting untrusted content. On AutoDAN-style inputs it reports Recall/ASR 92.5–98.0%, Precision 98.99–100%, FPR 0–2% across three backends (DeepSeek-V3.2, GPT-4o, GPT-5.2), lifting GPT-5.2 recall from 59.6% (single-pass baseline) to 98%. **Relevance to our lines (two-fold):** (a) *Decode-gap (Paper C):* RLM-JB is one of the few deployed defenses with an explicit **de-obfuscation step — but only for surface obfuscation (base64-like spans), not semantic encodings** (set theory, formal logic, classical Chinese). It therefore *reinforces* rather than closes the decode gap: a defense that decodes base64 still has no step that recovers a *semantically* encoded payload. (b) *Best-of-N canonicalization (candidate Paper D):* RLM-JB is the closest prior art to "canonicalization as a jailbreak defense," but its canonicalization is **ad-hoc preprocessing hygiene** inside a detection pipeline aimed at AutoDAN / long-context / split-payload attacks — it does **not** model the *budget/resampling (best-of-N)* structure, offers no effective-N / work-factor account, and is not evaluated against a resampling adversary. It confirms input canonicalization exists as a component (a novelty caveat any canonicalization-defense paper must clear) but does not preempt the effective-N-reduction framing, the augmentation carve, or the adaptive work-factor measurement. By its own limitations, it is untested against fully adaptive attackers that optimize against the detector and adds up to ~3× latency.
...
**ECSO — Eyes Closed, Safety On** (Gou et al., ECCV 2024) is the closest existing work to our IRC defense and the most threatening prior baseline. ECSO is a training-free, inference-time defense for MLLMs that **adaptively transforms unsafe images into text** before routing through the pre-aligned LLM's intrinsic safety mechanisms. The procedure: the MLLM inspects its own response for harmfulness signals; if a response is flagged unsafe, ECSO converts the input image to a textual description and re-queries the LLM, this time leveraging the LLM's text-side safety. Reported results: 37.6% safety improvement on MM-SafetyBench (SD+OCR) and 71.3% improvement on VLSafe with LLaVA-1.5-7B, with utility on standard MLLM benchmarks preserved. **Critical positioning for our IRC:** ECSO operates in the *opposite direction* — image→text via captioning, exploiting the LLM's text-side safety. Our IRC operates text→image, exploiting the VLM's image-side safety. The two defenses target complementary attack regimes. Crucially, ECSO assumes the text-side is the safer modality (the standard assumption pre-2024); our work hypothesizes that for *encoded* text, the image-side is safer because the VLM's image safety pathway operates on the visual gestalt of the prompt before OCR decoding, while the text-side has already been bypassed by encoding. Empirically demonstrating that IRC outperforms ECSO on text-encoding attacks (set theory, classical Chinese, formal logic) is critical: if ECSO suffices, IRC has no contribution. Conversely, if ECSO fails on encoded prompts because OCR + captioning preserves the encoding, our IRC fills a real gap.

**AdaShield** (Wang et al., ECCV 2024) defends MLLMs against structure-based jailbreak attacks (where harmful text is injected into images, e.g., FigStep) by prepending an **adaptive shield prompt** to the input. An auto-refinement framework iteratively generates context-specific defense prompts via a defender LLM that communicates with the target MLLM. The defense is inference-time, training-free, and architecturally cheap (a few hundred prompt tokens). **Relevance:** AdaShield is the simplest possible multimodal defense to add as a baseline — implementation cost is essentially zero. It targets the same attack class as ECSO (image-rendered harmful text) but uses prompt engineering rather than image-to-text transformation. Whether AdaShield can defend against semantically-encoded image-rendered text (our IRC's target attack regime) is uncharacterized; it likely fails since the shield prompt is designed for natural-language harmful intent that becomes visible after OCR — encoded prompts remain semantically opaque even after OCR succeeds.

**JailGuard** (Zhang et al., ACM TOSEM 2025) is a universal detection framework that **mutates untrusted inputs** across both text and image modalities and uses *response discrepancy* as the attack signal — benign inputs produce stable outputs under mutation while adversarial inputs produce divergent outputs. 18 mutators are implemented across text/image with a combination policy for cross-modal coverage. On a 15-attack benchmark, JailGuard achieves 86.14% / 82.90% detection accuracy on text/image inputs respectively, outperforming SOTA by 12–25 pp. **Relevance:** JailGuard is the canonical multimodal-detection baseline and a natural comparison for any new black-box VLM defense. Its core mechanism — mutation-based discrepancy detection — is a general approach that should in principle apply to our encoded image inputs, but the mutator library is built around natural-language and natural-image perturbations. Whether these mutators meaningfully perturb the semantic content of mathematical or Classical Chinese encoded prompts remains an open empirical question: if a Caesar-cipher rendering is mutated by character-substitution, the encoding may collapse and detection may succeed; if mutated by pixel noise, the encoding survives and detection likely fails.

**DefenSee** (Wang, Fok, & Thing, 2025) proposes a **multi-view defensive pipeline** for multimodal jailbreaks. The defense generates multiple visual variants of the input image, transcribes each via OCR/captioning, and performs cross-modal consistency checks across the views to detect attack inputs. The defense is black-box and inference-time. **Relevance:** DefenSee is the most recent multimodal defense and represents where the field is moving — image-variant ensemble + consistency rather than single-pass detection. Like ECSO it relies on OCR/captioning being effective on the input; whether the OCR pathway preserves enough semantic content of our encoded image prompts to flag inconsistency across views is empirically unknown. If our encoding survives OCR but remains semantically opaque, DefenSee's consistency check will pass and the defense will fail.

**HyPE / HyPS** (Maljkovic et al., 2026) leverages the geometry of **hyperbolic space** for harmful prompt detection (HyPE) and sanitization (HyPS). The detector models benign prompts in a hyperbolic embedding and flags outliers; the sanitizer uses attribution methods to identify and rewrite harmful words. **Relevance:** HyPE/HyPS targets the embedding-distribution structure of harmful content, similar in spirit to EDDF and semantic codebooks. As with those defenses, our semantically-encoded prompts likely have low similarity to canonical harmful prompts in any geometric embedding (Euclidean or hyperbolic) — the encoded surface form is far from the canonical harmful distribution even though the underlying intent is harmful. We expect HyPE to flag encoded prompts at near-baseline rates, providing another point of evidence that distribution-based defenses are structurally vulnerable to semantic encoding.

**SALLIE** (Azov, Rivlin, & Shtar, 2026) is an inference-time detection framework grounded in **mechanistic interpretability**: it extracts attack signals directly from the model's internal residual-stream activations and trains a lightweight k-NN classifier on them. **Relevance:** SALLIE represents the activation-based detection paradigm for multimodal attacks. The mechanism only applies in white-box settings (requires activation access), unlike our black-box IRC. Conceptually, however, SALLIE's premise — that attacks leave detectable activation signatures — is consistent with Wei et al.'s JRS finding that vision-pathway jailbreaks shift representations toward a "jailbreak direction." Whether encoded-image attacks (which arrive through the vision pathway) produce SALLIE-detectable signatures or evade them is an open question for white-box settings; our IRC sidesteps this by being black-box.

**Test-Time Attention Purification (CleanSight)** (Zhang et al., 2026) is **specifically a backdoor defense** for LVLMs that have been poisoned with trigger-bearing samples during fine-tuning, *not* a jailbreak defense in the conventional sense. The mechanistic insight: triggers operate by causing abnormal cross-modal attention redistribution, and CleanSight detects and prunes high-attention visual tokens at test time. **Relevance:** CleanSight is included for completeness as a recent inference-time VLM defense, but its attention-pruning mechanism is not designed for the jailbreak inputs we study (which lack the trigger-induced attention concentration that CleanSight exploits). It is not a relevant evaluation baseline for our attack but contextualizes the broader landscape of test-time VLM safety interventions and the increasing focus on attention-based defenses.

**Directional Embedding Smoothing (RESTA-VLM)** (Wang, Liu, & Koike-Akino, ICLR 2026 Workshop on Agents in the Wild) extends the **RESTA (Randomized Embedding Smoothing and Token Aggregation)** smoothing defense from text-only LLMs to VLMs. The defense injects directional noise aligned with the original token embedding vectors at the embedding layer and aggregates predictions across noisy copies. Evaluated on JailBreakV-28K. **Relevance:** This is the canonical multimodal extension of SmoothLLM/DR-Smoothing and a natural smoothing baseline for our setting. As with all smoothing defenses, the open question is whether the embedding noise affects semantically-encoded content (mathematical notation, formal logic): if the noise leaves the structural form of the encoding intact, the underlying meaning is preserved and the defense fails. We expect this to be the case for high-resolution encoded image inputs where the smoothing operates at the embedding level rather than the semantic level.
...
Chen et al. (2026) provide the most recent unified survey, **"Jailbreaking LLMs & VLMs: Mechanisms, Evaluation, and Unified Defense"**, arguing that jailbreak vulnerabilities stem from *structural* factors — incomplete training data, linguistic ambiguity, and generative uncertainty — rather than surface-level prompt patterns. The survey organizes the field through a three-dimensional framework spanning attacks, defenses, and evaluation metrics, and articulates unified defense principles across perception, generation, and parameter layers. **Relevance:** Useful for citing the cross-modal defense taxonomy and for framing our IRC defense within a broader mechanistic landscape — particularly the survey's argument that defenses should target *structural* rather than *surface* vulnerabilities, which aligns with our framing that encoding × modality attacks exploit a structural alignment gap rather than surface-level lexical patterns.
...
### 6.11 Concurrent work: guards vs. encoding/modality attacks — positioning (2025–2026)

A focused 2025–2026 sweep found that our *motivating claim* — no single deployed guard covers the union of the encoding and modality axes — has been substantially demonstrated by concurrent work. This **sharpens rather than undercuts** our positioning: the open contribution is the defense *architecture*, not the gap itself.

**MultiGuard (Verma et al., EMNLP 2025)** [circulated earlier as the "OmniGuard" preprint, arXiv:2505.23856] is the closest prior work and must be cited and differentiated explicitly. It measures deployed guards stratified by *both* our axes: LlamaGuard-3's catch rate falls 82% → 76% → 24.7% as a request shifts high-resource → low-resource → cipher-encoded text, and LlavaGuard averages ~2.3% catch on combined encoding-in-image cases (their own guard drops to 0.40% on base64-typographic images). This is essentially our empirical motivation — deployed guards collapse on the encoding∘modality union — established externally across multiple guards and both axes. **Differentiation:** MultiGuard responds by *training a new guard classifier*; it does **not** build a black-box recover(OCR)→decode→guard *pipeline* over existing target models. Our contribution is therefore the architecture, with MultiGuard as external corroboration of the gap rather than a claim we must independently establish.

**OmniGuard (Zhu et al., 2025; arXiv:2512.02306; `zhu2025omniguardunifiedomnimodalguardrails`) — a *different* paper from MultiGuard despite the shared name** (a citation hazard: 2505.23856 is MultiGuard@EMNLP-2025, 2512.02306 is this one — always pin the arXiv ID). It is a *trained* omni-modal **reasoning** guard: SFT of Qwen2.5-Omni-7B/3B on 210K curated uni- and cross-modal samples with distilled reasoning critiques, covering text/image/video/audio across 15 moderation benchmarks (BeaverTails, ToxicChat, WildGuard, VLGuard, UnsafeBench, MM-SafetyBench, …). **Crucially for us, its evaluation carries no encoding axis and no adversarial-placement axis:** no base64 / math / formal-logic encodings, no ASCII art, no typographically-rendered (OCR-recoverable) text-in-image, and no cross-modal *split* — its "cross-modal" cases are semantically-coherent unsafe pairings (an intent + a scene image), and it reports no adversarial-robustness metric. So it **inspects and reasons about intent; it does not *decode* an obfuscated payload.** **Positioning:** OmniGuard crowds the *"unified multimodal guard"* headline but does **not** touch our contribution — both differentiators we already draw against MultiGuard apply verbatim: it *trains a classifier* (vs. our black-box recover→decode→guard pipeline over the deployed target) and it *inspects rather than decodes*. It is therefore a **candidate breadth baseline**: run on our encoding∘modality suite it should fail (it never trained on encoded or rendered-text attacks), turning "even the newest SOTA trained omni-modal reasoning guard doesn't decode" into positive evidence that the decode gap is *architectural*, not a matter of training scale. Baseline-vs-cite-only is gated on the GPU cost of serving a 7B omni-model.

**CAMO (Jiang et al., 2025)** is the attack-side analogue: it encodes harmful text as a meaning-preserving math puzzle rendered into an image and reports ~100% evasion of a perplexity filter, an OCR-based toxicity classifier, and OpenAI's Moderation API — corroborating that image-rendered semantic encoding defeats deployed moderation. It is attack-only (no defense proposed).

**Encoding-axis-only results (text, no image)** further populate the gap: **Logic Jailbreak** (Peng et al., ACL Findings 2026) shows formal-logic-encoded prompts bypass LlamaGuard; **Trojan-Speak** (Sel et al., 2026) reports 99%+ evasion of Anthropic's Constitutional Classifiers via an adversarially fine-tuned cipher; and **Bypassing Prompt Guards in Production** (Fairoze et al., 2026) evades 14 open-weight guard models via controlled-release prompting. Together they show the encoding axis is well-populated; what remains thin is the *compound* encoding∘modality axis paired with a *defense*.

**The complication to address head-on:** **Jailbreaking VLMs Through the Visual Modality** (Azulay et al., 2026) shows a lightweight *output-side* guard (Qwen3Guard-Stream) *does* catch most compliant outputs of an image-rendered "visual cipher" attack. So our claim must be scoped: it is *input-side / full multimodal guards* that fail to cover the union — output-side response monitoring is a distinct, partially effective defense surface we should acknowledge and, ideally, include as a comparison point.

**Net positioning (implication for the paper; the final framing is a separate decision).** Because the audit-of-guards framing is now largely occupied (MultiGuard did it), the paper's contribution is best framed as the **coverage-complete recover→decode→guard defense**, motivated by the externally-established gap (MultiGuard, CAMO) and scoped by the output-side complication (Azulay et al.). SingGuard's taxonomy is supporting evidence the community already names this axis.

**Update (2026-07-11, OmniGuard #2).** The *unified / omni-modal guard* label is now dense — OmniGuard (2512.02306), UniGuard, AMIA, BlueSuffix, Qwen3Guard, GuardReasoner-VL, SingGuard — so "we build THE unified guard" is no longer a defensible headline. The headline must lead on the **decode** step specifically: recover-and-*decode*-then-judge, the one operation none of these inspect/reason guards perform, with "coverage-complete / unified" demoted to a framing device. This is a *strengthening*, not a retreat — every new inspect-only guard is one more confirmation that the decode gap is architectural, so the crowding cuts in our favor once the framing leads with decode.
...
### 7.5 Judge Reliability and Validated Evaluators (added 2026-07-12)

Every absolute ASR number in this study rests on an automated LLM judge (GPT-5-nano), and a concentrated body of 2024–2026 work shows that automated jailbreak judges are the least reliable component of safety evaluation — making judge choice a first-class methodological concern rather than an implementation detail.

**StrongREJECT** (Souly et al., NeurIPS 2024) is the strongest published response and our primary reference. It argues that jailbreak papers routinely overstate attack success because the field lacks a standard, high-quality evaluator, and contributes a 313-prompt dataset of *answerable* forbidden prompts together with an automated evaluator that scores `(1 − refused) × (specific + convincing)/2` — modeling both the model's *willingness* and the response's *usefulness* rather than mere non-refusal. Against a ground truth of five human labelers over 1,361 prompt–response pairs, its evaluators achieve the best agreement of any method tested: Spearman r = 0.846 for the GPT-4o-mini rubric judge and 0.900 for a fine-tuned Gemma-2B classifier, edging out HarmBench's own classifier (0.819) and vastly exceeding a GPT-4 policy judge (0.157), the OpenAI Moderation API (−0.103), and string matching (−0.394). Two facts make it directly relevant to us. First, its evaluator was validated on *encoded* attacks — the 17-method human-evaluation set includes Base64, ROT-13, disemvoweling, and low-resource translation — the closest published precedent to our own symbolic/semantic encoders, and it maintains near-best agreement across them. Second, the paper documents that existing evaluators *overstate* jailbreak effectiveness relative to humans (positive bias on nearly every baseline judge), the same inflation direction we observe with GPT-5-nano; it also reports a novel confound — jailbreaks that bypass safety fine-tuning tend to *degrade* the victim's capabilities, so a "successful" bypass often yields a less useful (hence less harmful) response than a binary judge assumes. The one caveat for our setting: StrongREJECT explicitly limits its scope to text LLMs and states it is "unclear whether StrongREJECT would be an appropriate benchmark for multimodal models." Its evaluator scores the text *response* (which our pipeline always produces, regardless of whether the input was image-rendered), so it applies mechanically to our VLM outputs — but its published validation does not itself extend to that setting, which motivates our own calibration below.

**SORRY-Bench** (Xie et al., ICLR 2025) complements this with a 44-category safety taxonomy, a class-balanced 440-prompt base set with 8.8K linguistic mutations, and a 7,040-annotation meta-evaluation of judge designs. Its adopted production judge — a fine-tuned Mistral-7B-Instruct-v0.2 — reaches 81.0% Cohen's κ with human labels (its fine-tuned GPT-3.5 variant reaches the top 83.8%, which the paper calls "almost perfect agreement"), one of the few benchmarks to report κ rather than raw agreement. Directly germane to us, SORRY-Bench's mutation study includes an encoding/encryption family (ASCII, Caesar, Morse, Atbash) and finds that most models "fail to understand or execute these encoded or encrypted unsafe instructions," so fulfillment drops 15–68% — except GPT-4o, whose fulfillment *rises* 11–16% on two of four schemes. This is a text-only analog of exactly our concern: encoding can mask harm from both the model and the judge, and its effect is sharply model-dependent.

A cluster of recent preprints establishes judge unreliability as an acknowledged, unsolved field-wide problem (we cite these as context and motivation, not as evaluators we adopt). **PandaGuard** (Shen et al., 2025) reports rule-based-vs-LLM judge agreement of only κ = 0.071–0.126 across a unified 49-model harness, finds that refusal-string matching consistently inflates ASR relative to content-aware LLM judges, and explicitly names multimodal-attack judging as unaddressed future work. **"A Coin Flip for Safety"** (Schwinn et al., 2026) human-audits 6,642 samples and finds that under the distribution shifts inherent to adversarial evaluation, four off-the-shelf safety judges perform "on average only slightly better than a random coin-flip" (AUROC 0.48–0.64; R² of 0.00–0.11 with human ratings), and shows a headline reversal in which Best-of-N flips from the *best*-reported to the *worst* attack once ASR is corrected for judge precision. **"How Reliable Is Your Jailbreak Judge?"** (Gao, 2026 — a single-author, AI-assisted preprint we weight cautiously) reports that LLM-as-judge models combine high precision (0.81–0.94) with very low recall (as low as 0.059), so they systematically miss real successes, and that content-preserving benign-framing wrappers flip LLM-judge verdicts 57–100% of the time (a human audit confirms the flipped responses still contain genuine harm); a fine-tuned HarmBench classifier is far more robust to these wrappers (3.4% flip rate) but over-flags (~16% false-positive inflation).

**Judges also *over*-count, and are alarmingly configuration-sensitive.** The failure is bidirectional. **VENOM** (Yan et al., Findings of EMNLP 2025) decouples a jailbreak's *technique* from whether the response actually holds genuine actionable harm, and shows mainstream LLM-as-judge frameworks key on malicious *tone* rather than factual content: under a controlled corruption protocol that strips real harmful content while preserving tone, false-positive rates reach **36–100%** across five published judge frameworks (JailbreakBench's binary judge is worst at ~100%; an atomic answer-vs-refuse detector is best). Independently, **Zhang** ("How Sensitive Are Safety Benchmarks to Judge Configuration Choices?", ICIC 2026) holds the judge *model* fixed and varies only the judge *prompt wording*: semantically-equivalent rewordings swing HarmBench harmful-rates by up to **24.2 points** and flip mid-tier model rankings on up to **83.6%** of bootstrap samples — variance "comparable to changing the judge model." Two lessons we adopt: report a judge *range*, not a scalar, and treat the judge prompt as a first-class experimental variable. Our own concern — judges *under*-counting decoded harm — is the mirror image of VENOM's over-counting, and both stem from the same root cause: judges keying on shallow surface signal instead of recovered semantic content.

**Decode-then-judge is emerging prior art — and we differentiate from it.** The closest work is **DecipherGuard** (Yang et al., 2025), an input-side guardrail that reverses obfuscation before classifying: a *rule-based* cascade decodes Base64, brute-forces Caesar shifts, and translates Zulu, then a LoRA-fine-tuned Llama-Guard-3-8B labels the deciphered text (raising detection from 2.6%→98.7% on Base64, 4.8%→84.9% on Caesar). The **JAM** attack (Jin et al., NeurIPS 2024) is likewise driven to 0% success by an analogous countermeasure — a secondary LLM that strips JAM's fixed cipher pattern, then judges the decoded text. Both validate the *principle* that recovery-before-judgment closes an evasion gap, but both are narrow where our approach is general: their decoders are **rule-based over a closed, hand-built set** of encodings (they explicitly do not handle set theory, formal logic, classical Chinese, or images), they operate as **input gates** that block rather than as response-side evaluators, and DecipherGuard **requires fine-tuning** its guard. Our recover+decode step is instead an *LLM-based, encoding-agnostic, semantic* recovery that needs no per-encoding rule and no guard fine-tuning — the exact generality gap these systems leave open, and the sharpest way to position our contribution against them.

**The open guard/judge landscape, and the multilingual gap our Chinese encoder exposes.** A large menu of open-weight safety judges now exists: beyond the classifier baselines (HarmBench-cls, Llama-Guard-3/4, WildGuard, MD-Judge), reasoning-augmented guards (**GuardReasoner** and its VL/Omni variants; **ThinkGuard**) and multilingual guards (**PolyGuard**, 17 languages; **Qwen3Guard**, 119) are the strongest recent additions — a third-party head-to-head of 14 open guards puts Qwen3Guard and NVIDIA Nemotron-safety at the top and OpenAI's gpt-oss-safeguard at the bottom. The multilingual evidence is a direct warning for our classical-Chinese arm: **PL-Guard** (Krasnodębska et al., Slavic NLP 2025) shows a small *native-language* classifier beats far larger general guards under perturbation (while GPT-4.1-nano collapses to an always-"unsafe" degenerate, macro-F1 0.438), and **CHILLGuard** (Yu et al., 2026), a dedicated Chinese guardrail, outperforms even Qwen3Guard on Chinese — yet CHILLGuard covers **only modern vernacular Chinese (白话文) and contains zero classical Chinese (文言文)**. No guard or judge in the literature has been validated on classical-Chinese-encoded harm, leaving that arm of our study uncharted. Finally, work on *reasoning-level* safety (**SaRO**, Findings of ACL 2026; **IPO**, ICLR 2026) reinforces the same intuition from another angle — safety judged on a model's *surface* output diverges from the safety of its intermediate *content* (IPO reports GPT-4o↔human agreement of 88% on responses but a noisier signal on reasoning traces) — echoing our claim that harm must be judged on *recovered content*, not surface form.

**Implications for our study.** These papers cut two ways. The *relative* contrasts our claims rest on — e.g. an off-the-shelf guard alone blocking 0–15% of encoded attacks versus 49–80% under our recover+decode amplifier — are robust to judge choice, because both conditions are scored by the same judge and the gap dwarfs any plausible judge error. What is fragile is only the *absolute* ASR levels. Our resulting measurement strategy, consistent with the published guidance: (1) keep HarmBench and JailbreakBench as prompt sources — no benchmark ships a fresh, strong, human-validated judge for our exact setting, so switching benchmarks buys nothing, and we do not adopt any *unpublished* benchmark as an evaluation instrument; (2) re-derive headline numbers under StrongREJECT's peer-reviewed, human-validated evaluator, reporting GPT-5-nano and the HarmBench classifier alongside as a spread; and (3) run our own human-agreement calibration (Cohen's κ) on a stratified sample of our outputs. This third step is not merely diligence: none of these five works — nor any benchmark we surveyed — validates a judge on image-rendered or symbolic-encoded (set-theory, formal-logic, classical-Chinese) attacks, so a human-calibrated judge for this setting is a small contribution in its own right, and it is what extends StrongREJECT's text-only validation to our multimodal outputs. Finally, FigStep's finding (§8) that HarmBench's classifier *under*-counts ASR on typographic/image-rendered attacks — the opposite direction from GPT-5-nano's over-count — is the sharpest evidence that judges misfire inconsistently on precisely our attack class, and we cite it directly. Two further consequences: our recover+decode amplifier is positioned against the decode-then-judge prior art (DecipherGuard, JAM) on *generality* — LLM-based, encoding-agnostic, response-side, no fine-tuning — rather than on the principle itself; and our judge-validation (Round J, `experiments_plan.md`) bakes off the open-guard landscape mapped above (see `judge_candidates.md`) against human labels on our *own* encoded/rendered outputs, since none of these judges has been validated there and — per CHILLGuard — the classical-Chinese case is untouched.
...
DeepSeek-OCR adds a fourth motivation: optical text is becoming a plausible **efficiency mechanism**, not only an adversarial surface. If future long-context systems compress dialogue histories, documents, or memory into image-like representations, then representation-dependent safety is no longer a niche jailbreak concern. It becomes a general question for multimodal system design: safety behavior must remain calibrated when text moves between digital tokens and optical/vision tokens.
...
7. **Safety under optical context compression.** Inspired by DeepSeek-OCR, future work should test whether text compressed through OCR-like image representations preserves safety decisions. This extends beyond jailbreak images: a model may treat optically compressed conversation history differently from tokenized history, especially when old context is downsampled, blurred, or partially reconstructed.

---

## 9. Coverage-Complete Defense — scoping-search synthesis (2026-06-11)

> Added after the direction shift to the **coverage-complete defense** framing. This section is deliberately scoped to what §0–§8 do *not* already cover: (i) the **portfolio / best-of-suite threat model** and its adversarial-ML methodological lineage, (ii) the **multi-surface / "unified" defense** category and the closest prior art to our guard, (iii) the **cross-modal-splitting** surface that bounds per-channel coverage, and (iv) a **reuse-vs-invent** verdict for the attack side. Citations use live `bibkey`s from `paper/literature/my_base.bib`.

### 9.1 The coverage-map lens and the portfolio (best-of-suite) threat model

The project recasts defenses by the **surface their safety check covers** — input-text, input-image, output, or the joint (text+image) interpretation — and attacks by **where they place harm**. A defense reduces an attack's success only when its coverage includes the surface that attack uses; each deployed defense is a *specialist*, so against a suite of attacks placing harm on different surfaces, no single specialist covers the union.

The threat model is **best-of-suite**: the attacker draws from a fixed public suite and succeeds if *any* member defeats the deployed defense. This is not an inflated metric but the *reliable* one, and it has a direct adversarial-ML pedigree. **AutoAttack** (Croce & Hein, ICML 2020; `3524938.3525144`) established that an ensemble of diverse, parameter-free attacks gives a trustworthy robustness *lower bound* that hand-tuned single attacks miss — it lowered reported robust accuracy on 50+ models, exposing many defenses as overestimated. **RobustBench** (`croce2021robustbench`) operationalized this as a leaderboard under a *pinned, declared* threat model. The jailbreak transplants are **HarmBench** (`10.5555/3692070.3693501`) and **JailbreakBench** (`NEURIPS2024_63092d79`); **Best-of-N Jailbreaking** (Hughes et al., NeurIPS 2025; `NEURIPS2025_69f3eb24`) formalizes any-success-counts aggregation over *stochastic samples of one template* (cross-sample). Crucially, the **cross-method** form ("any of several *distinct* attacks succeeds") is **also already standard** — it is exactly AutoAttack's worst-case-over-an-ensemble protocol, and HarmBench/JailbreakBench aggregate ASR over methods by default. The best-of-suite operator therefore carries **no novelty in either form** (cross-sample or cross-method); we **cite it as established methodology** and claim nothing for the aggregation itself. Its role in this project is purely as the **deployment-realistic threat model** that makes "no single specialist covers the union" a meaningful measurement — the contribution lives entirely in the *defense*, not the portfolio.

The load-bearing methodological lesson is the **within-scope rule**. Tramèr et al. (NeurIPS 2020; `NEURIPS2020_11f38f8e`), Obfuscated Gradients (Athalye et al., ICML 2018; `obfuscated-gradients`), and Carlini & Wagner's detection-bypass study (AISec 2017; `10.1145/3128572.3140444`) all show that a "vulnerability" is only evidence against a defense when the attack lies **inside that defense's declared threat model**; an out-of-scope or under-powered attack yields a *false* reading. This cuts two ways for us: (a) the portfolio's cross-surface "exposure" of a single-surface specialist is **out-of-scope as an attack claim** (a strawman a reviewer would reject) but is **exactly the right motivation for a defense paper** — the same measurement, opposite fate under framing; and (b) to claim *our* guard is robust we must run **in-scope adaptive attacks** against it, not just replay the static suite (cf. Andriushchenko et al., ICLR 2025; `andriushchenko2025jailbreaking`). Chouldechova et al. (NeurIPS 2025; `chouldechova2025comparison`) adds the caution that ASR comparisons require valid, matched measurement.

### 9.2 Multi-surface / "unified" defenses — the closest prior art

Most deployed black-box defenses are single-surface specialists: **SAGE** (`ding-etal-2025-act`) and **SemanticSmooth** (`ji-etal-2025-defending`) on input-text; **ECSO** (`10.1007/978-3-031-72643-9_23`) converting image→text but **gated on `has_image`**; **ETA** (`ding2025eta`) on image (CLIP) + output (reward); **MLLM-Protector** (`DBLP:conf/emnlp/PiHZXPLDZZ24`) on output; **AdaShield** (`10.1007/978-3-031-72661-3_5`) on the image via a defense prompt; and the detectors **CIDER** (`xu-etal-2024-cross`), **JBShield** (`307822`), and **JailDAM** (`nian2025jaildam`).

The defenses that most resemble a *single guard over the union* — and are therefore the closest prior art our contribution must distinguish from — are **AMIA** (Zhang et al., EMNLP Findings 2025; `zhang-etal-2025-amia`), which masks text-irrelevant image patches **and** runs joint intention analysis (image + joint surfaces, inference-only, reporting a utility cost: DSR 52.4%→81.7%, ~2% accuracy drop), and **BlueSuffix** (Zhao et al., ICLR 2025; `zhao2025bluesuffix`), which combines a visual purifier + a textual purifier + an RL-trained suffix (both input channels). Complementing these, the **textual-unlearning transfer** result (Chakraborty et al., EMNLP 2024; `chakraborty-etal-2024-textual`) shows a *text-space* intervention drives vision-text ASR below ~8% — evidence the modalities share a fused safety locus, which is the mechanistic reason a union check can work.

**Our defensible novelty against this prior art:** a *single black-box, training-free* guard that (i) recovers and checks content on **both** input-text and input-image in **one** unified pass, (ii) removes the `has_image` gate that ECSO/decoy-style levers exploit, and (iii) treats **over-refusal cost as a first-class, quantified axis**. None of AMIA (masks + analysis, image/joint only), BlueSuffix (trains the suffix), or the output-side methods delivers all three simultaneously. The gap is real but narrow — the related-work section must engage AMIA and BlueSuffix head-on.

### 9.3 The cross-modal-splitting surface — the limit of per-channel coverage

The threat that defeats even a coverage-complete *per-channel/union* check is **cross-modal splitting**: harm present only in the joint text+image interpretation, with each channel benign. This is well-populated and citable: **MML** (Wang et al., ACL 2025; `wang-etal-2025-jailbreak`) splits via an encryption–decryption process across modalities with an "evil-alignment" game-dev frame (99.4% ASR on GPT-4o); **SI-Attack** (Zhao et al., ICCV 2025; `zhao2025siattack`) exploits a comprehension-vs-safety gap by shuffling text+image instructions; **Jailbreak-in-Pieces** (Shayegani et al., ICLR 2024; `shayegani2024jailbreak`) pairs a benign text prompt with an embedding-space–optimized toxic image. Image-fragment-only siblings are **SIVA** (`rashid2026robustnessvisionlanguagemodels`) and **MIDAS** (`liu2026midas`).

A distinction that matters for what our guard can claim: some "splits" are **semantic** (the union of recovered text/captions is harmful — a union OCR/caption check recovers them) while others are **steganographic or embedding-space** (harm hidden below the recoverable layer — invisible to OCR/captioning, so a union check *misses* them). The coverage-complete guard's honest scope is **semantic** cross-channel placement; steganographic/embedding splits are a separate, harder regime and should be named as a boundary, motivating joint multimodal verification (the §11.2 future-work line) rather than over-claimed.

### 9.4 Reuse-vs-invent verdict and positioning

**Attack side — reuse, do not invent.** Both the cross-modal *split* (MML, SI-Attack, Jailbreak-in-Pieces) and the *compound single-input* attack (Arondight, `10.1145/3664647.3681379`; ArtPrompt, `jiang-etal-2024-artprompt`) already exist as citable, often code-released inputs. Constructing a new one risks a "you reinvented MML / Jailbreak-in-Pieces" rejection; the contribution should be the **defense** (the coverage-complete guard, and eventually joint verification), with these attacks folded in as off-the-shelf suite members.

**Where this project sits.** The portfolio measurement **motivates** ("no single specialist covers the union"); the coverage-complete guard is the **minimal black-box fix** that beats every single-surface specialist *by construction*, so the genuine unknown — and the reportable result whatever its value — is the **over-refusal cost** of completeness. Cross-modal splitting is the acknowledged **boundary** that bounds per-channel coverage and points to joint verification. **MMJ-Bench** (`10.1609/aaai.v39i26.34983`) is the precedent for unified attack/defense evaluation on a common bench, and the *From LLMs to MLLMs* survey (`wang-etal-2024-llms-mllms`) supplies the input/encoder/generator/output taxonomy our **surface-based coverage map** refines.
...
- Kumarappan, A. & Mehrotra, A. *Towards Realistic Guarantees: A Probabilistic Certificate for SmoothLLM.* NeurIPS Lock-LLM Workshop, 2025. arXiv:2511.18721.
...
- Azov, G., Rivlin, O. & Shtar, G. *SALLIE: Safeguarding Against Latent Language & Image Exploits.* arXiv:2604.06247, 2026.
- Chen, Z. et al. *Jailbreaking LLMs & VLMs: Mechanisms, Evaluation, and Unified Defense.* arXiv:2601.03594, 2026.
- Gou, Y. et al. *Eyes Closed, Safety On: Protecting Multimodal LLMs via Image-to-Text Transformation (ECSO).* ECCV, 2024.
- Maljkovic, I. et al. *Harnessing Hyperbolic Geometry for Harmful Prompt Detection and Sanitization (HyPE / HyPS).* ICLR, 2026.
- Tan, R.Y., Hu, Y. & Lee, R.K.-W. *Structured Visual Narratives Undermine Safety Alignment in Multimodal Large Language Models (ComicJailbreak).* arXiv:2603.21697, 2026.
- Wang, Y., Liu, X., Li, Y., Chen, M. & Xiao, C. *AdaShield: Safeguarding Multimodal Large Language Models from Structure-Based Attack via Adaptive Shield Prompting.* ECCV, 2024.
- Wang, Y., Liu, J. & Koike-Akino, T. *Directional Embedding Smoothing for Robust Vision Language Models (RESTA-VLM).* ICLR Workshop on Agents in the Wild, 2026.
- Wang, Z., Fok, K.W. & Thing, V.L.L. *DefenSee: Dissecting Threat from Sight and Text — A Multi-View Defensive Pipeline for Multi-modal Jailbreaks.* arXiv:2512.01185, 2025.
- Zhang, X. et al. *JailGuard: A Universal Detection Framework for Prompt-based Attacks on LLM Systems.* ACM TOSEM, 2025.
- Zhang, Z. et al. *Test-Time Attention Purification for Backdoored Large Vision Language Models (CleanSight).* arXiv:2603.12989, 2026.
