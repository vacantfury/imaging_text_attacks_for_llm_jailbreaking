...
*Shared across ALL papers that report HarmBench ASR — Paper B (image_presence_threshold), Paper C
...
`judge_model_issue/` (labels stay local); this doc records the design and the realized
composition.
...
- **κ is reported sliced** — ALL / representative / disagreement — per judge
  (`judge_model_issue/compute_kappa.py`). HarmBench-cls only ran on some arms, so its κ
...
- **Universe & reproducibility.** Drawn from 20,584 unique stored responses
  ...
  `judge_model_issue/build_sheet.py` regenerates the identical sample.
...
`python3 judge_model_issue/build_sheet.py` (prints the composition summary).
...
- Reproduce: verdicts in `judge_model_issue/round_j_candidate_verdicts.csv` +
  `round_j_cluster_verdicts.json`; human labels `round_j_judge_model_issues.csv`;
  table via `judge_model_issue/compute_consensus.py`.
