...
**Workflow stage:** S4 · literature/scoop loop — **scoop-check #1 + full deep-read DONE; verdict FIRMED at Level 3 (Medium), delta SAFE** (2026-07-19; §4, prior art in `literature_review.md §14`). Both High-risk candidates defanged on inspection. **Order from here (owner-corrected 2026-07-19 — keep this sequence):** (1) preliminary proposal ✅ → (2) S4 literature / scoop search ✅ → (3) `idea_check.md` ✅ (`text_docs/agent_injection/idea_check.md`, grounded in §14; owner hands → cspaper.org/idea-check) → (4) **NEXT: S5 main story** (lock the delta + positioning) → (5) S6 design + cost. Paper E, off the July AAAI crunch (Papers C/D own that); targets a later cycle (§8).
