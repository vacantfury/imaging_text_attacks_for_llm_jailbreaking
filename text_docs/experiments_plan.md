...
**▶ IN FLIGHT — IMAGE round LAUNCHED 2026-07-11 as `mj_8296134`** (campaign `paper_c_image`): the make-or-break modality-coverage test — `modality_complete` RECOVER+DECODE(union)+GUARD vs `no_defense`/`ecso`, × {set_theory, formal_logic} `ir_plain` × 3 open VLMs, 24 cells. NOTE: fixed `modality_complete` to decode the text+recovered-image UNION (commit `ddaa7b6`) — else the recovered image transcription stays encoded and fools the guard. Running → **triage with `check-experiment-results`**, watching: `no_defense` ASR (did OCR transmit the attack?), `mc-on` vs `mc-off` (does union-decode close the image gap?), over-refusal. Find via cluster `squeue` or `run_index.py --campaign paper_c_image`. Clear once triaged.
