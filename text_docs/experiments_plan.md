...
**▶ Round 3 IN FLIGHT (`mj_8303802`, campaign `paper_c_round3_amplifier`) — submitted 2026-07-12, awaiting triage.** The REFRAMED headline (commit `6e07023`): recover+decode as an AMPLIFIER prepended to an OFF-THE-SHELF guard. 45 cells (HarmBench 0–99): `{no_defense, guard_baseline[guardreasoner_vl_7b], modality_complete[guard_model=guardreasoner_vl_7b, decode]}` × `{set_theory, formal_logic, code_attack (text); set_theory, formal_logic (ir_plain)}` × `{qwen2_5_vl_7b, internvl3_8b, pixtral_12b}`. **FIRST real cluster run** of the guard serve-plumbing + `guard_baseline` / mc-external-guard machinery (commit `cc3c503`) + `guardreasoner_vl_7b` (a Qwen2.5-VL-7B fine-tune → low serve risk; wildguard/llama_guard_4 EXCLUDED tonight, validated separately). Budget: 4 gpu servers = MaxJobsPU cap exactly (submitted with 0 competing jobs), orchestrator on `short`. **Next action (cold-resume): sync remote→local, then triage (`check-experiment-results`).** WATCH: (1) did `guardreasoner_vl_7b` SERVE (no endpoint hang)? — if the WHOLE job hung, this is why; (2) does `guard_baseline[gr]` MISS (high ASR) where `modality_complete[gr]` CATCHES (low ASR)? spot-read raw; (3) `parse_guardreasoner_vl` sanity on real `<think>/<result>` output (fail-closed on unparseable → watch artifact over-block / suspiciously low ASR); (4) runtime: guardreasoner 4096-tok traces on ~30 cells — partial cells are per-cell isolated, not a failed round; (5) in-run `no_defense` should reproduce the Round-1b/2img baselines (sanity anchor). Grid target: a new `experiment_results.md` "Round 3 — amplifier × guard" table.
