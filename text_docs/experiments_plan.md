...
⏭️ **NEXT make-or-break: the IMAGE channel** — `modality_complete`'s OCR-recover + decode on `ir_plain`-rendered encoded attacks (the actual modality-coverage thesis, still UNTESTED), then Round 2 benign-refusal COST + the consensus rejudge for headline numbers. Open risks that could still sink/reframe the paper: image channel fails, over-refusal cost too high, adaptive/harder encoders + cross-modal splitting, positioning (decode vs modality framing).
