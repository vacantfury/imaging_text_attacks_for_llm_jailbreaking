...
2. **Judge validity (one-time, cheap):** the entire coverage map rests on the `gpt-5-nano` judge, so calibrate it once. Hand-label a **stratified ~40-row sample** of stored target responses (mix of refusals / compliances / borderline, spread across encoders × defenses) and report judge↔human agreement (accuracy + Cohen's κ) on HarmBench-success and JBB-refusal. Raw verdicts are already stored → no target re-query. Low agreement caveats every ASR number; report it once and move on.
