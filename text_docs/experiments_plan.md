...
**▶ IN FLIGHT — Round 1b LAUNCHED 2026-07-11 as `mj_8295436`** (campaign `paper_c_round1b`): 30 cells — **(A)** refined `modality_complete` {decode on/off} × 3 encoders × 3 open VLMs (design test: does decode rescue mc?); **(B)** {no_defense, amia_ia, mc-on, mc-off} × 3 encoders × `gemini-2.5-flash` (cheap capability probe: does decode-then-judge work on a capable model?). Judge gpt-5-nano. Running → **triage with `check-experiment-results`**, spot-reading the DECODE-step outputs. Find via cluster `squeue` or `run_index.py --campaign paper_c_round1b`. Clear once triaged.
