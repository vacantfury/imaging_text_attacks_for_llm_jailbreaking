...
**▶ IN FLIGHT — Round 1 LAUNCHED 2026-07-11 as `mj_8291382`** (campaign `paper_c_round1`): the 36-cell text-only decode-gap gate — `{set_theory, formal_logic, code_attack}` × `{no_defense, sage, amia_ia, modality_complete}` × `{qwen2_5_vl_7b, internvl3_8b, pixtral_12b}`, HarmBench 0–99, judge gpt-5-nano. Running on the cluster, awaiting completion → **triage with `check-experiment-results`** (first real run of sage/amia_ia/modality_complete — spot-read raw). A fresh session finds it via cluster `squeue` (while running) or `run_index.py --campaign paper_c_round1` (once outputs pulled). Clear this marker once triaged + recorded to `experiment_results.md`.
