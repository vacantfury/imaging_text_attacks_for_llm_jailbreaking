...
**Cluster sync (family standard, settled 2026-08-02):** the canonical convention is git clone/pull for committed code + rsync/scp for the gitignored ops layer (`scripts/`, `conf/cluster_pool.yaml`, `cluster_files/`) + rsync-down for results — science organ `knowledge/cluster_sync_convention.md`. **This repo does NOT yet conform:** its cluster checkouts are rsync-everything / non-git (never a designed choice — no rationale was ever documented). Migrate to the hybrid (git-clone the code path) when the repo is next substantially touched; not urgent — the current setup works. Do not fork a new scheme.
